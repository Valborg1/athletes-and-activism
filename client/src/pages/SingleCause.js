...
export default function SingleCause() {
    ...
    function loadSingleCause() {
        API.getSingleCause(id).then((res) => {
            console.log("single cause", res.data);
            return res.data;
        }).then((res) => setSingleCause(res)).catch((err) => console.log(err));
    }

    function loadCauseAthletes() {
        let supportingAthletes = [];

        API.getCauseAthletes(id).then((res) => {
        ...
        })).then((res) => setCauseAthletes(supportingAthletes)).catch((err) => console.log(err));
    }

    function loadCauseCharities() {
        API.getCauseCharities(id).then((res) => {
            console.log("cause charities", res.data);
            return res.data;
        }).then((res) => setCauseCharities(res)).catch((err) => console.log(err));
    }
